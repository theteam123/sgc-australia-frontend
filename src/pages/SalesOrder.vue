<template>
  <div class="container">
    ...
    <div class="document-header">
      ...
      <div class="header-section">
        <div class="header-title-section">
          ...
          <p class="page-subtitle">Manage sales orders for {{ projectName }}</p>
        </div>
    ...
    <div v-else class="documents-section">
      ...
      <div class="summary-cards">
        <div class="summary-card">
          ...
          <div class="summary-content">
            <div class="summary-number">{{ salesOrders.length }}</div>
      ...
      <!-- Sales Orders List -->
      <div class="documents-list" v-if="salesOrders.length > 0">
        <div class="list-header">
          <h3 class="list-title">Sales Orders</h3>
        ...
        <div class="table-container hidden md:block">
          <table class="documents-table">
            ...
            <tbody>
              <tr v-for="order in filteredSalesOrders" :key="order.name" class="table-row">
                <td>
      ...
      <div v-else class="empty-state">
        ...
        <h3 class="empty-title">No Sales Orders Found</h3>
        <p class="empty-description">
          No sales orders have been created for this project yet.
        </p>
        <button 
          @click="createNewSalesOrder"
          class="btn-primary"
...
import { getProjectDocuments, createSalesOrder } from '../services/documents';
...
const submittedOrders = computed(() => 
  salesOrders.value.filter(order => order.docstatus === 1).length
);

const draftOrders = computed(() => 
  salesOrders.value.filter(order => order.docstatus === 0).length
);

const totalValue = computed(() => 
  salesOrders.value.reduce((sum, order) => sum + (order.grand_total || 0), 0)
);
...
const loadSalesOrders = async () => {
  ...
  try {
    ...
    if (projectData.value.name) {
      const orders = await getProjectDocuments('Sales Order', projectData.value.name);
...
const createNewSalesOrder = () => {
  ...
  router.push(`/sales-order/new-sales-order-${formId}?project=${encodeURIComponent(projectData.value.name)}&projectId=${encodeURIComponent(projectId.value)}`);
};

const viewSalesOrder = (order: SalesOrder) => {
  // Navigate to ERPNext document view
  const baseUrl = getErpNextUrl();
  window.location.href = `${baseUrl}/app/sales-order/${order.name}`;
};
...
const downloadPDF = (order: SalesOrder) => {
  // Navigate to ERPNext PDF download
  const baseUrl = getErpNextUrl();
  window.open(`${baseUrl}/app/sales-order/${order.name}/print`, '_blank');
};
