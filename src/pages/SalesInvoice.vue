<template>
  <div class="container">
    ...
    <div class="document-header">
      ...
      <div class="header-section">
        <div class="header-title-section">
          ...
          <p class="page-subtitle">Manage sales invoices for {{ projectName }}</p>
        </div>
        <div class="header-actions">
          <button 
            @click="createNewSalesInvoice"
            class="btn-primary"
            :disabled="loading"
          >
    ...
    <div v-else class="documents-section">
      ...
      <div class="summary-cards">
        ...
        >
          ...
          <div class="summary-content">
            <div class="summary-number">{{ salesInvoices.length }}</div>
      ...
      <!-- Sales Invoices List -->
      <div class="documents-list" v-if="salesInvoices.length > 0">
        <div class="list-header">
          <div class="list-title-section">
            <h3 class="list-title">Sales Invoices</h3>
            <div v-if="statusFilter !== 'all'" class="filter-indicator">
              <span class="filter-label">Filtered by:</span>
              <span class="filter-value">{{ statusFilter.charAt(0).toUpperCase() + statusFilter.slice(1) }}</span>
              <button @click="filterByStatus('all')" class="clear-filter-btn" title="Clear filter">
                ×
              </button>
            </div>
          </div>
        ...
        <div class="table-container hidden md:block">
          <table class="documents-table">
            ...
            <tbody>
              <tr v-for="invoice in filteredSalesInvoices" :key="invoice.name" class="table-row">
                <td>
                ...
                <td>
                  <span class="text-sm">{{ invoice.sales_order || 'N/A' }}</span>
                </td>
                <td>
                ...
                <td>
                  <div class="actions-cell">
                    <button 
                      @click="viewSalesInvoice(invoice)"
                    ...
                    <button 
                      @click="editSalesInvoice(invoice)"
        ...
        <div class="mobile-cards md:hidden">
          <div 
            v-for="invoice in filteredSalesInvoices" 
            :key="invoice.name" 
            class="mobile-card"
          >
            ...
            <div class="mobile-card-content">
              <div class="mobile-card-row">
                <span class="mobile-card-label">Sales Order:</span>
                <span>{{ invoice.sales_order || 'N/A' }}</span>
              </div>
              <div class="mobile-card-row">
      ...
      <div v-else class="empty-state">
        ...
        <h3 class="empty-title">No Sales Invoices Found</h3>
...
import { getProjectDocuments, createSalesInvoice } from '../services/documents';
...
const filteredSalesInvoices = computed(() => {
  ...
  if (statusFilter.value !== 'all') {
    if (statusFilter.value === 'draft') {
      filtered = filtered.filter(invoice => (invoice.docstatus || 0) === 0);
    } else if (statusFilter.value === 'submitted') {
      filtered = filtered.filter(invoice => (invoice.docstatus || 0) === 1);
    }
  }
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(invoice => 
      ...
      (invoice.sales_order && invoice.sales_order.toLowerCase().includes(query))
    );
  }
...
const submittedInvoices = computed(() => 
  salesInvoices.value.filter(invoice => invoice.docstatus === 1).length
);

const draftInvoices = computed(() => 
  salesInvoices.value.filter(invoice => invoice.docstatus === 0).length
);

const totalValue = computed(() => 
  salesInvoices.value.reduce((sum, invoice) => sum + (invoice.grand_total || 0), 0)
);
...
const loadSalesInvoices = async () => {
  ...
  try {
    ...
    if (projectData.value.name) {
      const invoices = await getProjectDocuments('Sales Invoice', projectData.value.name);
      salesInvoices.value = invoices || [];
    }
  } catch (err: any) {
    console.error('Error loading sales invoices:', err);
    error.value = err.message || 'Failed to load sales invoices';
  } finally {
    loading.value = false;
  }
};
...
const createNewSalesInvoice = () => {
  ...
  router.push(`/sales-invoice/new-sales-invoice-${formId}?project=${encodeURIComponent(projectData.value.name)}&projectId=${encodeURIComponent(projectId.value)}`);
};

const viewSalesInvoice = (invoice: SalesInvoice) => {
  // Navigate to ERPNext document view
  const baseUrl = getErpNextUrl();
  window.location.href = `${baseUrl}/app/sales-invoice/${invoice.name}`;
};
...
const downloadPDF = (invoice: SalesInvoice) => {
  ...
  const baseUrl = getErpNextUrl();
  window.open(`${baseUrl}/app/sales-invoice/${invoice.name}/print`, '_blank');
};

const filterByStatus = (status: string) => {
  statusFilter.value = status;
  console.log(`Filtering sales invoices by status: ${status}`);
};

const showTotalValueDetails = () => {
  // Show detailed breakdown of total value
  const breakdown = salesInvoices.value.map(invoice => ({
    name: invoice.name,
    customer: invoice.customer,
    total: invoice.grand_total || 0
  }));
  console.log('Total value breakdown:', breakdown);
  alert(`Total Value Details:\n${breakdown.map(b => `${b.name}: ${formatCurrency(b.total)}`).join('\n')}`);
};
...
.clickable-card.active .summary-number {
  color: var(--primary-green);
}

.clickable-card.active .summary-label {
  color: var(--primary-green);
  font-weight: 600;
}
