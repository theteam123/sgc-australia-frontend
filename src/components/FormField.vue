  ...
  <div :class="[
    ...
    <template v-if="field.fieldtype === 'Section Break'">
      <div class="mt-8 mb-4 border-b border-gray-200 pb-2">
        <span v-if="field.label" class="text-h3">{{ formattedLabel }}</span>
      </div>
    </template>
    ...
    <template v-else-if="field.fieldtype === 'Data'">
      <div class="w-full lg:w-1/2">
        <label :for="field.fieldname" class="form-label" :class="{ 'form-label-required': isFieldRequired }">
          {{ formattedLabel }}
        </label>
        ...
        <p v-if="field.description" class="text-small">
          {{ field.description }}
        </p>
      </div>
    </template>
